...
*snps extracted 26 Feb 2021 from new qc'd dataset with txt in this script: /tsd/p471/data/durable/projects/mh_bmi/scripts/colossus/manual_adultbmi_prs.sh
...
*import and prep:
import delimited "genetic/clumped_moba_adultbmi_snps_allinfo.txt", case(preserve) clear 
...
drop N idexposure
save "data/formatted_adultbmisnps", replace
...
*this runs from 0.02 to 0.97. fine. 
...
*first, record which ones:
list SNP if palindrome==1
/*   |        SNP |
     |------------|
 61. | rs10824218 |
 73. | rs10887578 |
...
sort SNP
save data/moba_gwas_snplevelinfo_adultbmi.dta, replace
...
import delimited "N:\data\durable\projects\mh_bmi\genetic\adultbmi_snps.raw", varnames(nonames) clear
...
import delimited "N:\data\durable\projects\mh_bmi\genetic\adultbmi_snps.raw", clear
...
save "scratch/adultbmi_genetic.dta", replace

*****************

*NOW MAKE THE PGS.

use "scratch/adultbmi_genetic.dta", clear
...
list SNP harm_beta_gwas palindrome in 1/929

*save the order of snps in a global for later:
levelsof SNP, local(adultbmi_snps)
display "adultbmi_snps"
local adultbmi_snplist_temp: subinstr local adultbmi_snps "char(34)" "char(32)", all
local adultbmi_snplist_temp2: subinstr local adultbmi_snplist_temp "rs" "  rs", all
global adultbmi_snplist: subinstr local adultbmi_snplist_temp2 "  " ""
...
replace adultbmi_pgs=adultbmi_pgs+(`relevantbeta'*0) if `var'==0
replace adultbmi_pgs=adultbmi_pgs+(`relevantbeta'*1) if `var'==1
replace adultbmi_pgs=adultbmi_pgs+(`relevantbeta'*2) if `var'==2
*for people for whom the snp is missing, ADD BETA*MEAN VALUE
sum `var'
return list
replace adultbmi_pgs=adultbmi_pgs+(`relevantbeta'*r(mean)) if `var'==.
}

*standardize and rename it for consistency:
egen z_adultbmi_pgs=std(adultbmi_pgs)
summ z_adultbmi_pgs

drop _merge
save "scratch/adultbmi_genetic_pgs.dta", replace
...
clear
*/
use "scratch/adultbmi_genetic_pgs.dta", clear
capture drop _merge
...
merge 1:1 iid using "scratch/adultbmi_genetic_pgs.dta"
keep if _merge==3
drop _merge
save "scratch/linkable_adultbmi_genetic_pgs.dta", replace
...
*renpfix rs c_rs
*rename adultbmi_pgs c_adultbmi_pgs
*rename z_adultbmi_pgs c_z_adultbmi_pgs
...
keep if role=="Father"
renpfix rs f_rs
rename adultbmi_pgs f_adultbmi_pgs
rename z_adultbmi_pgs f_z_adultbmi_pgs
...
keep if role=="Mother"
renpfix rs m_rs
rename adultbmi_pgs m_adultbmi_pgs
rename z_adultbmi_pgs m_z_adultbmi_pgs
...
*and with the scores:
pwcorr z_adultbmi_pgs m_z_adultbmi_pgs  f_z_adultbmi_pgs 
...
save "scratch/reshaped_linkable_adultbmi_genetic_pgs.dta", replace

*****************************************************************************************************
*suspect there are space issues, so erase intermediate files?
*not the second - need them for the SNP-level robustness checks:
capture erase "scratch/adultbmi_genetic_pgs.dta"
*capture erase "scratch/linkable_adultbmi_genetic_pgs.dta"
******************************************************************************************************
*****************************************************************************************************
...
cd "N:\data\durable\projects\mh_bmi\"
use "scratch/reshaped_linkable_adultbmi_genetic_pgs.dta", replace
...
save "data\statafiles\geno_pheno_adultbmi.dta", replace
**************************************************************************************************
**************************************************************************************************
...
use "data\statafiles\geno_pheno_adultbmi.dta", clear

*COMPLETE-CASE ANALYSIS:

*how much of bmi does the pgs predict?

regress moffspring_bmi_Q8y c_z_adultbmi_pgs 
regress mothers_bmi_Q1 m_z_adultbmi_pgs
regress fathers_bmi_Q1QF f_z_adultbmi_pgs
...
eststo `var'_mr: ivreg `var' (moffspring_bmi_Q8=c_z_adultbmi_pgs) KJONN c_batch_* if fullgenetic==1 & fullccbmi==1
*with parents genetic -  iv with pgs
eststo `var'_wfmr: ivreg `var' (moffspring_bmi_Q8y mothers_bmi_Q1 fathers_bmi_Q1QF = c_z_adultbmi_pgs m_z_adultbmi_pgs f_z_adultbmi_pgs) KJONN c_batch_* m_batch_* f_batch_* if fullgenetic==1 & fullccbmi==1
}
