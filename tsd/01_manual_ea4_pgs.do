...
*import and prep:
import delimited "data/snplevel_info/clumped_moba_ea4_snps_allinfo.txt", case(preserve) clear 
...
save "data/snplevel_info/formatted_ea4snps", replace
...
sort SNP
save "data/snplevel_info/moba_gwas_snplevelinfo_ea4.dta", replace
...
import delimited "data/genetic/ea4_snps.raw", varnames(nonames) clear
...
import delimited "data/genetic/ea4_snps.raw", clear
...
save "scratch/ea4_genetic.dta", replace

*****************

*NOW MAKE THE PGS.

use scratch/ea4_genetic.dta, clear
...
merge 1:1 _n using data/snplevel_info/moba_gwas_snplevelinfo_ea4.dta

list SNP harm_beta_gwas palindromic in 1/1730
*yep

*save the order of snps in a global for later:
levelsof SNP, local(ea4_snps)
display "ea4_snps"
local ea4_snplist_temp: subinstr local ea4_snps "char(34)" "char(32)", all
local ea4_snplist_temp2: subinstr local ea4_snplist_temp "rs" "  rs", all
global ea4_snplist: subinstr local ea4_snplist_temp2 "  " ""
display $ea4_snplist

*check it works in a loop:
foreach snp in $ea4_snplist {
	display "`snp'"
}
...
capture drop ea4_pgs*
gen ea4_pgs=0 
*if batch_num!=.
*nb: keeping as . for people who weren't genotyped (FIX THIS LATER)
local k=0
foreach var in $ea4_snplist {
display "`var'"
fre `var'_
local k=`k'+1
local relevantbeta=harm_beta_gwas in `k'
display `relevantbeta'
replace ea4_pgs=ea4_pgs+(`relevantbeta'*0) if `var'_==0
replace ea4_pgs=ea4_pgs+(`relevantbeta'*1) if `var'_==1
replace ea4_pgs=ea4_pgs+(`relevantbeta'*2) if `var'_==2
*for people for whom the snp is missing, ADD BETA*MEAN VALUE
sum `var'_
return list
replace ea4_pgs=ea4_pgs+(`relevantbeta'*r(mean)) if `var'_==.
}

*standardize and rename it for consistency:
egen z_ea4_pgs=std(ea4_pgs)
summ z_ea4_pgs

drop _merge
save "scratch/ea4_genetic_pgs.dta", replace

************************************************************************************************************
*ADD GENETIC COVARIATES:
*In new release, PCs are part of the covariate file now, so don't need an individual step to add them.

use "scratch/ea4_genetic_pgs.dta", clear
...
rename *pgs* c_*pgs*
drop role
save "scratch/c_ea4_genetic_pgs.dta", replace
restore
...
rename *pgs* m_*pgs*
drop role
save "scratch/m_ea4_genetic_pgs.dta", replace
restore
...
rename *pgs* f_*pgs*
drop role
save "scratch/f_ea4_genetic_pgs.dta", replace
restore

*now attach all three to the SV_INFO file,
*then merge these all together using the SV_INFO file.
*NB: need to first increase maximum variables - not the case for other scores but this has the most SNPs
...
count
*112,265
merge 1:m PREG_ID_2306 using "scratch/c_ea4_genetic_pgs.dta"
rename _merge _merge1
*N increased here because of multiple births, presumably (1:m merge)
count
*112,996
merge m:1 M_ID_2306 using "scratch/m_ea4_genetic_pgs.dta"
rename _merge _merge2
merge m:1 F_ID_2306 using "scratch/f_ea4_genetic_pgs.dta"
...
*SAVE IT:
*drop _merge
save "scratch/reshaped_linkable_ea4_genetic_pgs.dta", replace

************************************************
*MENDELIAN ERRORS CHECK:

use "scratch/reshaped_linkable_ea4_genetic_pgs.dta", clear
...
gen m_ea4_snp_coeff=.
gen f_ea4_snp_coeff=.
levelsof ea4_snp, local(levels) clean
local k=0
foreach snp of local levels {
local k=`k'+1
reg c_`snp'_ f_`snp'_ m_`snp'_
replace m_ea4_snp_coeff=_b[m_`snp'_] in `k'
replace f_ea4_snp_coeff=_b[f_`snp'_] in `k'
}
*then summ these:
summ m_ea4_snp_coeff, det
summ f_ea4_snp_coeff, det
*not sure exactly what counts as an outlier, but looks ok

*ditto for the scores:
pwcorr c_z_ea4_pgs m_z_ea4_pgs  f_z_ea4_pgs 
