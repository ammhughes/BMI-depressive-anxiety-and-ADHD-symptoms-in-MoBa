...
*import and prep:
import delimited "data/snplevel_info/clumped_moba_adultbmi_snps_allinfo.txt", case(preserve) clear 
...
drop N idexposure
save "data/snplevel_info/formatted_adultbmisnps", replace
...
sort SNP
save data/snplevel_info/moba_gwas_snplevelinfo_adultbmi.dta, replace
...
import delimited "data\genetic\adultbmi_snps.raw", varnames(nonames) clear
...
import delimited "data\genetic\adultbmi_snps.raw", clear
...
*'merge' in snplevel info, even though not really a merge
*in the other file, data needs to be sorted by SNP so this comes out in the right order.
...
*center
regress z_adultbmi_pgs PC* 
estimates store a
regress z_adultbmi_pgs PC*  i.genotyping_center_num
estimates store b
lrtest b a
*no: - Prob > chi2 = 0.7300
*chip
regress z_adultbmi_pgs PC* 
estimates store a
regress z_adultbmi_pgs PC*  i.genotyping_chip_num
estimates store b
lrtest b a
*not quite: Prob > chi2 = 0.0512
*genotyping batch:
regress z_adultbmi_pgs PC* 
estimates store a
regress z_adultbmi_pgs PC*  i.genotyping_batch_num
estimates store b
lrtest b a
*definitely: Prob > chi2 = 0.0000
*imputation batch/release:
regress z_adultbmi_pgs PC* 
estimates store a
regress z_adultbmi_pgs PC*  i.imputation_batch_num
estimates store b
lrtest b a
*no: Prob > chi2 = 0.1385
...
rename *pgs* c_*pgs*
drop role
save "scratch/c_adultbmi_genetic_pgs.dta", replace
restore
...
rename *pgs* m_*pgs*
drop role
save "scratch/m_adultbmi_genetic_pgs.dta", replace
restore
...
rename *pgs* f_*pgs*
drop role
save "scratch/f_adultbmi_genetic_pgs.dta", replace
restore
...
count
*112,265
merge 1:m PREG_ID_2306 using "scratch/c_adultbmi_genetic_pgs.dta"
rename _merge _merge1
*N increased here because of multiple births, presumably (1:m merge)
count
*112,996
merge m:1 M_ID_2306 using "scratch/m_adultbmi_genetic_pgs.dta"
rename _merge _merge2
merge m:1 F_ID_2306 using "scratch/f_adultbmi_genetic_pgs.dta"
...
gen m_adultbmi_snp_coeff=.
gen f_adultbmi_snp_coeff=.
levelsof adultbmi_snp, local(levels) clean
local k=0
foreach snp of local levels {
local k=`k'+1
reg c_`snp' f_`snp' m_`snp'
replace m_adultbmi_snp_coeff=_b[m_`snp'] in `k'
replace f_adultbmi_snp_coeff=_b[f_`snp'] in `k'
}
*then summ these:
summ m_adultbmi_snp_coeff, det
summ f_adultbmi_snp_coeff, det
*looks ok

*ditto for the scores:
pwcorr c_z_adultbmi_pgs m_z_adultbmi_pgs  f_z_adultbmi_pgs 
