*ea4excl23andme - SCORE MADE IN STATA
...
*get updated ea4excl23andme snps:
...
*import and prep:
import delimited "data/snplevel_info/clumped_moba_ea4excl23andme_snps_allinfo.txt", case(preserve) clear 
rename effect_allele effect_allele_gwas
rename other_allele other_allele_gwas
*even though the eaf was from the hrc not the gwas, rename consistent with the other traits:
rename EAF_HRC effect_allele_freq_gwas
...
drop seexposureunadj pvalexposureunadj idexposure
save "data/snplevel_info/formatted_ea4excl23andmesnps", replace
...
*this runs from 0.01 to 0.99. fine. 
...
tab eaf_alignment palindromic, row chi
*as expected
...
sort SNP
save "data/snplevel_info/moba_gwas_snplevelinfo_ea4excl23andme.dta", replace
...
import delimited "data/genetic/ea4excl23andme_snps.raw", varnames(nonames) clear
...
import delimited "data/genetic/ea4excl23andme_snps.raw", clear
...
save "scratch/ea4excl23andme_genetic.dta", replace

*****************

*NOW MAKE THE PGS.

use scratch/ea4excl23andme_genetic.dta, clear
...
merge 1:1 _n using data/snplevel_info/moba_gwas_snplevelinfo_ea4excl23andme.dta

list SNP harm_beta_gwas palindromic in 1/672
*yep

*save the order of snps in a global for later:
levelsof SNP, local(ea4excl23andme_snps)
display "ea4excl23andme_snps"
local ea4excl23andme_snplist_temp: subinstr local ea4excl23andme_snps "char(34)" "char(32)", all
local ea4excl23andme_snplist_temp2: subinstr local ea4excl23andme_snplist_temp "rs" "  rs", all
global ea4excl23andme_snplist: subinstr local ea4excl23andme_snplist_temp2 "  " ""
display $ea4excl23andme_snplist

*check it works in a loop:
foreach snp in $ea4excl23andme_snplist {
	display "`snp'"
}
...
local k=0
foreach var in $adultbmi_snplist {
local k=`k'+1
display `var'
fre `var'
count if `var'==.
...
*NB: need the underscore after `var' in the loop below because otherwise there are two variables for which the rsid in the global is an ambiguous abbreviation.
capture drop ea4excl23andme_pgs*
gen ea4excl23andme_pgs=0 
*if batch_num!=.
*nb: keeping as . for people who weren't genotyped (FIX THIS LATER)
local k=0
foreach var in $ea4excl23andme_snplist {
display "`var'"
fre `var'_
local k=`k'+1
local relevantbeta=harm_beta_gwas in `k'
display `relevantbeta'
replace ea4excl23andme_pgs=ea4excl23andme_pgs+(`relevantbeta'*0) if `var'_==0
replace ea4excl23andme_pgs=ea4excl23andme_pgs+(`relevantbeta'*1) if `var'_==1
replace ea4excl23andme_pgs=ea4excl23andme_pgs+(`relevantbeta'*2) if `var'_==2
*for people for whom the snp is missing, ADD BETA*MEAN VALUE
sum `var'_
return list
replace ea4excl23andme_pgs=ea4excl23andme_pgs+(`relevantbeta'*r(mean)) if `var'_==.
}

*standardize and rename it for consistency:
egen z_ea4excl23andme_pgs=std(ea4excl23andme_pgs)
summ z_ea4excl23andme_pgs

drop _merge
save "scratch/ea4excl23andme_genetic_pgs.dta", replace

************************************************************************************************************
*ADD GENETIC COVARIATES:
*In new release, PCs are part of the covariate file now, so don't need an individual step to add them.

use "scratch/ea4excl23andme_genetic_pgs.dta", clear
...
*****************************************************************
*First, GLOBAL COVARIATE EFFECTS CHECK across everyone.
*check effects of all genetic covariates on the pgs, adjusted for PCs:
*center
regress z_ea4excl23andme_pgs PC* 
estimates store a
regress z_ea4excl23andme_pgs PC*  i.genotyping_center_num
estimates store b
lrtest b a
*no: - Prob > chi2 = 0.0972
*chip
regress z_ea4excl23andme_pgs PC* 
estimates store a
regress z_ea4excl23andme_pgs PC*  i.genotyping_chip_num
estimates store b
lrtest b a
*yes: Prob > chi2 = 0.0248
*genotyping batch:
regress z_ea4excl23andme_pgs PC* 
estimates store a
regress z_ea4excl23andme_pgs PC*  i.genotyping_batch_num
estimates store b
lrtest b a
*no: Prob > chi2 = 0.5598
*imputation batch/release:
regress z_ea4excl23andme_pgs PC* 
estimates store a
regress z_ea4excl23andme_pgs PC*  i.imputation_batch_num
estimates store b
lrtest b a
*no: Prob > chi2 = 0.2235
...
rename *pgs* c_*pgs*
drop role
save "scratch/c_ea4excl23andme_genetic_pgs.dta", replace
restore
...
rename *pgs* m_*pgs*
drop role
save "scratch/m_ea4excl23andme_genetic_pgs.dta", replace
restore
...
rename *pgs* f_*pgs*
drop role
save "scratch/f_ea4excl23andme_genetic_pgs.dta", replace
restore
...
count
*112,265
merge 1:m PREG_ID_2306 using "scratch/c_ea4excl23andme_genetic_pgs.dta"
rename _merge _merge1
*N increased here because of multiple births, presumably (1:m merge)
count
*112,996
merge m:1 M_ID_2306 using "scratch/m_ea4excl23andme_genetic_pgs.dta"
rename _merge _merge2
merge m:1 F_ID_2306 using "scratch/f_ea4excl23andme_genetic_pgs.dta"
...
*SAVE IT:
*drop _merge
save "scratch/reshaped_linkable_ea4excl23andme_genetic_pgs.dta", replace

************************************************
*MENDELIAN ERRORS CHECK:

use "scratch/reshaped_linkable_ea4excl23andme_genetic_pgs.dta", clear
...
gen ea4excl23andme_snp=""
local k=0
foreach snp in $ea4excl23andme_snplist {
local k=`k'+1
display "`snp'"
replace ea4excl23andme_snp="`snp'" in `k'
}
...
gen m_ea4excl23andme_snp_coeff=.
gen f_ea4excl23andme_snp_coeff=.
levelsof ea4excl23andme_snp, local(levels) clean
local k=0
foreach snp of local levels {
local k=`k'+1
reg c_`snp' f_`snp' m_`snp'
replace m_ea4excl23andme_snp_coeff=_b[m_`snp'] in `k'
replace f_ea4excl23andme_snp_coeff=_b[f_`snp'] in `k'
}
*then summ these:
summ m_ea4excl23andme_snp_coeff, det
summ f_ea4excl23andme_snp_coeff, det
*not sure exactly what counts as an outlier, but looks ok

*ditto for the scores:
pwcorr c_z_ea4excl23andme_pgs m_z_ea4excl23andme_pgs  f_z_ea4excl23andme_pgs 
