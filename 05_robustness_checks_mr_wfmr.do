...
*did this for the 314 + 928 SNPs in parallel on colossus because otherwise it would have taken weeks (approx 1hr15 for one SNP with 5 x outcomes and 2 x genetic models)
...
use data/moba_gwas_snplevelinfo_adultbmi.dta
list SNP harm_beta_gwas palindrome in 1/314
...
use "scratch/adultbmi_singlesnps/from_colossus/rs10007906_outcome_assoc.dta", clear

*append the rest:
foreach snp in $dultbmi_snplist {
append using "scratch/adultbmi_singlesnps/from_colossus/`snp'_outcome_assoc.dta"
}
*drop the duplicate of the first
duplicates drop
count
*928 SNPs
save "scratch/adultbmi_singlesnps/adultbmi_all_snps_outcome_assoc.dta", replace
...
use "scratch/adultbmi_singlesnps/adultbmi_all_snps_outcome_assoc.dta", clear

/*not in this version: trim off the un-asked for space whihc has been added: replace SNP = trim(SNP)*/
merge 1:1 SNP using "data/moba_gwas_snplevelinfo_adultbmi.dta"
drop _merge
...
*sysdir set PLUS "N:\data\durable\people\Mandy\statafiles\for_mrrobust"
*sysdir set PLUS "N:\data\durable\projects\mh_bmi\ado\plus"
*sysdir set PLUS "N:\data\durable\people\Mandy\statafiles\ado\plus"
...
*get the list of snps for child bmi:
use data/moba_gwas_snplevelinfo_childbmi.dta
list SNP harm_beta_gwas palindrome in 1/314
...
*start with the first in the list:
use "scratch/childbmi_singlesnps/from_colossus/rs10095724_outcome_assoc.dta", clear

*append the rest:
foreach snp in $childbmi_snplist {
append using "scratch/childbmi_singlesnps/from_colossus/`snp'_outcome_assoc.dta"
}
*drop the duplicate of the first
duplicates drop
count
*314 SNPs
save "scratch/childbmi_singlesnps/childbmi_all_snps_outcome_assoc.dta", replace

************************************************************************************************************

**MERGE BACK ON THE SNP-LEVEL INFO INCL EXTERNAL BETAS, MATCHING ON SNP

use "scratch/childbmi_singlesnps/childbmi_all_snps_outcome_assoc.dta", clear

/*not in this version: trim off the un-asked for space whihc has been added: replace SNP = trim(SNP)*/
merge 1:1 SNP using "data/moba_gwas_snplevelinfo_childbmi.dta"
drop _merge
list SNP harm_beta_gwas palindrome in 1/314
